...
function buildBoardItems(docs: DocForBoard[], folders: FolderForBoard[]): BoardItem[] {
  ...
  const items: BoardItem[] = [
    ...
    ...docs.filter((d) => !d.folder_id).map((d) => ({
      kind: 'doc' as const,
      id: d.id,
      doc: d,
    })),
        ...
        <Stack spacing={1.5}>
              ...
              {isAdmin && onCustomize && (
                <Tooltip title="Customize">
                  <IconButton
                    ...
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onCustomize(doc, e.currentTarget); }}
                  >
              ...
              {isAdmin && onRemoveFromFolder && (
                <Tooltip title="Remove from folder">
                  <IconButton
                    size="small"
                    sx={{ opacity: 0.45, '&:hover': { opacity: 1 } }}
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onRemoveFromFolder(doc); }}
                  >
          ...
          <Link href={`/docs/${doc.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
            <Stack spacing={1}>
              {doc.image && (
                <Box
                  component="img" src={doc.image} alt={doc.title}
                  sx={{ width: '100%', height: 130, objectFit: 'cover', borderRadius: 2, border: '1px solid rgba(12,123,220,0.08)' }}
                />
              )}
              <Stack direction="row" spacing={0.75} alignItems="flex-start">
                {CardIcon && (
                  <Box sx={{ color: accent || 'primary.main', mt: 0.25, flexShrink: 0 }}>
              ...
              >
                {createExcerpt(doc.description || doc.content)}
              </Typography>
  ...
  return (
    ...
    >
      <Box sx={{ p: 2.5, width: 270 }}>
        <Typography variant="subtitle2" fontWeight={700} gutterBottom>Accent color</Typography>
        <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap sx={{ mb: 2 }}>
          {CARD_COLORS.map((c) => (
            <Swatch key={String(c.value)} value={c.value} swatch={c.swatch} selected={color === c.value} onClick={() => setColor(c.value)} />
          ))}
        </Stack>
        ...
        <Stack direction="row" spacing={1} justifyContent="flex-end">
          ...
          <Button size="small" variant="contained" onClick={() => { onSave(doc.id, color, icon); onClose(); }}>Apply</Button>
        </Stack>
      </Box>
    </Popover>
  );
    ...
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        <TextField
          ...
          onKeyDown={(e) => { if (e.key === 'Enter' && name.trim()) { onSave(name.trim(), color, icon); onClose(); } }}
          sx={{ mt: 1, mb: 2.5 }}
        />
        <Typography variant="subtitle2" fontWeight={700} gutterBottom>Color</Typography>
        <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap sx={{ mb: 2.5 }}>
          {FOLDER_COLORS.map((c) => (
            <Swatch key={String(c.value)} value={c.value} swatch={c.value || '#9e9e9e'} selected={color === c.value} onClick={() => setColor(c.value)} />
          ))}
        </Stack>
        <Typography variant="subtitle2" fontWeight={700} gutterBottom>Icon</Typography>
      ...
      <DialogActions>
        ...
        <Button variant="contained" disabled={!name.trim()} onClick={() => { onSave(name.trim(), color, icon); onClose(); }}>Save</Button>
      </DialogActions>
    </Dialog>
  );
}
...
export function DocumentBoard({
  ...
  const saveOrder = useCallback((items: BoardItem[]) => {
  ...
  function handleCreateFolder(name: string, color: string | null, icon?: string | null) {
    ...
    fetch('/api/folders', {
      ...
      .then((folder: FolderForBoard) => {
        setBoardItems((prev) => [
          ...prev,
          { kind: 'folder', id: `folder:${folder.id}`, folder, docs: [] },
        ]);
      });
  }
  ...
  function handleRenameFolder(folder: FolderForBoard, name: string, color: string | null, icon: string | null) {
    setBoardItems((prev) =>
      prev.map((i) =>
        i.kind === 'folder' && i.folder.id === folder.id
          ? { ...i, folder: { ...i.folder, name, color, icon } }
          : i,
      ),
    );
  ...
  function handleDeleteFolder(folder: FolderForBoard) {
    ...
    setBoardItems((prev) => {
      const folderItem = prev.find(
        (i) => i.kind === 'folder' && i.folder.id === folder.id,
      ) as Extract<BoardItem, { kind: 'folder' }> | undefined;
      ...
      const next = [
        ...prev.filter((i) => !(i.kind === 'folder' && i.folder.id === folder.id)),
        ...freedDocs,
      ];
      saveOrder(next);
      return next;
        ...
        if (item.kind === 'folder') {
          return {
            ...
            docs: item.docs.map((d) =>
              d.id === id ? { ...d, card_color: color, card_icon: icon } : d,
            ),
          };
        }
        return item;
  ...
  return (
    <>
            ...
            {boardItems.map((item) => {
              ...
              return (
                <SortableFolderCard
                  ...
                  onRename={() => { setEditingFolder(item.folder); setFolderDialogOpen(true); }}
      ...
      <CardCustomizePopover
        ...
        onClose={() => { setCustomizeAnchor(null); setCustomizeDoc(null); }}
        onSave={handleCardCustomizeSave}
      />

      <FolderDialog
        open={folderDialogOpen}
        initial={editingFolder ? { name: editingFolder.name, color: editingFolder.color, icon: editingFolder.icon } : undefined}
